...
int i;
srand(time(NULL));
int Tmax=1e8;								/*Maximum number of time steps*/
int measures=1e4;							/*Measure variable*/
int Tint=Tmax/measures;							/*Measuring interval*/
...
FILE *nc;
char buffernc[96];
sprintf(buffernc, "nclusters_a%.3f_f%.3f_t%d_N%d.dat", alpha, fi, Tmax, N); /*Saves the total number of clusters*/
nc=fopen(buffernc, "wb");
FILE *snap;
char buffersnap[96];
sprintf(buffersnap, "system_a%.3f_f%.3f_t%d_N%d.dat", alpha, fi, Tmax, N); /*Saves the total number of clusters*/
...
for (step=0; step<Tmax; step++) {					/*Loop for time steps - OPEN*/
		...
		for (i=0; i<Nclusters; i++) {
			...
			if ((step+1)%Tint==0 && step>0) {Nsize[size-1]++;}	/*Save number of clusters of same size at the time intervals*/
			//if ((step+1)%Tint==0) {printf("Cluster %d Size %d N[%d]=%d\n",i,size,size,Nsize[size-1]);}
		...
		char message[96];
		sprintf(message,"%.2f %% elapsed",(step+1)/(double)Tmax*100);
		...
		if ((step)%(int)10==0) {fprintf(nc, "%d	%d\n", step+1, Nclusters);}			/*Save evolution of # of clusters*/

		/*int sn;
		for (sn=0; sn<N; sn++) {fprintf(snap, "%d ", array[sn]);}
		fprintf(snap, "\n");*/

		/*if (step+1==1) {printf("BEGINNING SIMULATION\n");}	
		if (step+1==0.01*Tmax) {printf("1 %% ELAPSED\n");}
		if (step+1==0.10*Tmax) {printf("10 %% ELAPSED\n");}
		if (step+1==0.25*Tmax) {printf("25 %% ELAPSED\n");}
		if (step+1==0.50*Tmax) {printf("50 %% ELAPSED\n");}
		if (step+1==0.75*Tmax) {printf("75 %% ELAPSED\n");}
		if (step+1==0.90*Tmax) {printf("90 %% ELAPSED\n");}*/
